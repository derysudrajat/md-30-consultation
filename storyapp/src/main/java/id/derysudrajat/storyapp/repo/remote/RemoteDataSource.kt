...
class RemoteDataSource @Inject constructor(private val storyService: StoryService) {
    ...
    suspend fun getAllStory(token: String, page: Int, size: Int) = storyService.getAllStory(token, page, size)
    ...
    suspend fun postNewStory(
        ...
        flow<States<String>> {
            emit(States.loading())
            val response = storyService.postNewStory(token, file, description, latLng.latitude, latLng.longitude)
            response.let {
                if (it.error != true) emit(States.success(it.message ?: ""))
                else emit(States.failed(it.message ?: ""))
            }
